...
<form method="POST" action="{{ $action }}" class="space-y-5 rounded-xl bg-white p-6 shadow">
    @csrf
    @if ($method !== 'POST')
        @method($method)
    @endif
    ...
    <div>
        ...
        <select name="type" id="type"
            class="w-full rounded-lg border border-gray-300 px-3 py-2 focus:border-blue-500 focus:outline-none">
            <option value="expense" @selected(old('type', $transaction?->type?->value) === 'expense')>Wydatek</option>
            <option value="income" @selected(old('type', $transaction?->type?->value) === 'income')>Przychód</option>
        </select>
        @error('type')
            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
        @enderror
    ...
    <div>
        ...
        <input type="date" name="occurred_on" id="occurred_on"
