...
└─ Typ C: WERYFIKACJA E2E (Weryfikacja: / oznaczenie 🌐)
   ...
   3. Re-uruchom weryfikacje wizualna PRZEZ agent-browser:
      - Ustal URL aplikacji (http://localhost:8080 — nginx; panel admina: http://localhost:8080/admin)
...
2. Jesli istnieje `composer.json` (projekt Laravel):
   - Testy: `php artisan test` (lub `vendor/bin/pest`, jesli zainstalowany Pest)
   - Lint/format: `vendor/bin/pint --test` (Laravel Pint)
   - Analiza statyczna: `vendor/bin/phpstan analyse` lub `vendor/bin/phpstan` (jesli jest larastan/phpstan)
   - Frontend: jak w punkcie `package.json` powyzej (`npm run build`, `vue-tsc`)
   - **Brak lokalnego PHP** — wszystkie komendy uruchamiaj przez kontener:
     `docker compose exec -T app <komenda>` (np. `docker compose exec -T app php artisan test`)
3. Jesli istnieje `Makefile` → uzyj `make <target>` (typecheck, test, build)
4. Jesli istnieje `pyproject.toml` → uzyj narzedzi Python (mypy, pytest, ruff)
5. Jesli istnieje `Cargo.toml` → `cargo check && cargo test && cargo build`
