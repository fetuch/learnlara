...
- `build-errors/` — błędy Composera, Vite, kompilacji assetów, konfiguracji build
- `runtime-errors/` — błędy w czasie wykonania, crashe, unhandled exceptions
- `eloquent-issues/` — problemy z bazą: migracje, relacje, zapytania, N+1, transakcje
- `auth-issues/` — problemy z autentykacją i autoryzacją (Sanctum, policies, gates)
- `ui-bugs/` — błędy wizualne, layout, responsywność, interakcje (Inertia/Vue)
- `filament-issues/` — panel admina: resources, forms, tables, actions, widgets
- `inertia-vue-issues/` — Inertia + Vue: props, formularze, SSR, hydration, routing
- `performance-issues/` — wolne zapytania, N+1, memory leaks, cache, kolejki
- `typescript-errors/` — błędy typów we frontendzie (Vue/TS), type assertions, generics
- `deployment-issues/` — problemy z wdrożeniem, Docker, CI/CD, environment
- `testing-issues/` — failing testy, konfiguracja testów (PHPUnit/Vitest), mocking
...
\```php
// kod rozwiązania (PHP/Laravel; dla frontendu użyj ```vue / ```ts)
\```
