...
8. Przypisz kazdemu sub-agentowi inny frame ideacji jako **bias startowy, nie ograniczenie**. Instruuj kazdego agenta, zeby zaczynal ze swojej perspektywy ale podazal za kazdym obiecujacym watek gdziekolwiek prowadzi — pomysly cross-cutting obejmujace wiele frame'ow sa cenne, nie poza scope'm.
   ...
   **4 frame'y ideacji (dostosowane do stacku Laravel/PHP/Inertia/Vue/Filament/PostgreSQL):**

   1. **Tech Debt Scout** — szuka:
      - typy `any` w TypeScript (frontend Vue), brak typów na propsach Inertia
      - komentarze TODO/FIXME
      - "fat controllers"/modele > 300 linii, logika w kontrolerach zamiast w serwisach/akcjach
      - brakujace testy (PHPUnit/Pest, Vitest)
      - łamanie konwencji Laravela (brak Form Requestów, brak `$fillable`, masowy `DB::` zamiast Eloquent)
      - nieaktualne zaleznosci (`composer outdated`, `npm outdated`)

   2. **UX Advocate** — szuka:
      - brak stanow empty/error/loading w komponentach Vue / stronach Inertia
      ...
      - niespojny design (niezgodnosc z design systemem / shadcn-vue)

   3. **Performance Analyst** — szuka:
      - zapytania N+1 (brak eager loading `with()`), ciężkie zapytania bez indeksów
      - brak cache (`Cache::`, cache zapytań/konfiguracji), brak kolejek dla wolnych operacji
      - rozmiar bundle (brak code splitting), brak lazy loading komponentów Vue
      - brak paginacji dużych zbiorów (też w tabelach Filamenta)
   ...
   4. **Product Strategist** — szuka:
      ...
      - brak monitoringu (Sentry dla Laravel, logowanie bledow, Pulse/Telescope)
      - brakujace feature'y wzgledem konkurencji
      - mozliwosci automatyzacji (scheduler/cron, joby kolejkowe, eventy/listenery)
